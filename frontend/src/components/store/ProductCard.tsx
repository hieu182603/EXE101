...
interface ProductCardProps {
  ...
  price: string;
  oldPrice?: string;
  tag?: string; // Expecting strings like "-18%" or "New"
  rating?: number;
  imageIndex?: number;
}

const ProductCard: React.FC<ProductCardProps> = ({ id, name, price, oldPrice, tag, rating = 5, imageIndex = 0 }) => {
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-1 mb-2">
          {[...Array(5)].map((_, i) => (
            <span key={i} className={`material-symbols-outlined text-[14px] ${i < rating ? 'text-yellow-400 fill' : 'text-zinc-700'}`}>star</span>
          ))}
        </div>
        ...
        <div className="mt-auto space-y-3">
          <div className="flex flex-col">
            {oldPrice && (
               <span className="text-xs text-zinc-500 line-through font-medium">{oldPrice}</span>
            )}
            <span className="text-xl font-black text-red-500 tracking-tight">{price}</span>
          </div>
