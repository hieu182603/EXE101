...
const ProfilePage: React.FC = () => {
  ...
  const saveAddress = () => {
    if (!addressForm.recipientName || !addressForm.phone || !addressForm.detail) {
      showError(t('profile.validation.addressRequired'));
      return;
    }
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-[1000px]">
        ...
        <div className="pt-20 px-8 pb-12 space-y-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="w-full md:w-auto">
              ...
              <p className="text-slate-500 text-sm font-medium">{t('profile.memberSince')}: {user.joinDate} • <span className="text-primary font-bold">{t('profile.memberId')}: {user.memberId}</span></p>
            </div>

            <div className="flex gap-3 flex-wrap">
              {isEditing ? (
                <>
                  <button onClick={cancelEditing} className="px-6 py-2.5 border border-border-dark rounded-xl text-sm font-bold text-slate-400 hover:text-white hover:bg-white/5 transition-all">{t('profile.actions.cancel')}</button>
                  <button onClick={saveChanges} className="px-6 py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:bg-red-600 transition-all shadow-lg shadow-primary/20">{t('profile.actions.saveChanges')}</button>
                </>
              ) : (
                <>
                  <button onClick={startEditing} className="px-5 py-2.5 border border-border-dark rounded-xl text-sm font-bold text-white hover:bg-white/5 transition-all flex items-center gap-2">
                    <span className="material-symbols-outlined text-[18px]">edit</span> {t('profile.actions.editProfile')}
                  </button>
                  <button onClick={() => setIsChangePasswordModalOpen(true)} className="px-5 py-2.5 border border-border-dark rounded-xl text-sm font-bold text-white hover:bg-white/5 transition-all flex items-center gap-2">
                    <span className="material-symbols-outlined text-[18px]">lock_reset</span> {t('profile.actions.changePassword')}
                  </button>
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('profile.labels.email')}</p>
                    {isEditing ? (
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('profile.labels.phone')}</p>
                    {isEditing ? (
              ...
              {/* Membership Section */}
              <div className="space-y-6">
                <h3 className="font-black text-white uppercase text-[10px] tracking-[0.2em] border-b border-border-dark pb-2">{t('profile.membership')}</h3>
                <div className="bg-background-dark/50 border border-border-dark rounded-2xl p-6 flex items-center gap-5 group hover:border-primary/30 transition-all">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-end mb-1">
                      <p className="text-lg font-black text-white">{t('profile.labels.rank')}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">1,500 / 2,000 {t('profile.labels.points')}</p>
                    </div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-wider">{t('profile.labels.pointsNeeded', { points: '500' })}</p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b border-border-dark pb-2">
                  <h3 className="font-black text-white uppercase text-[10px] tracking-[0.2em]">{t('profile.addresses')}</h3>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b border-border-dark pb-2">
                  <h3 className="font-black text-white uppercase text-[10px] tracking-[0.2em]">{t('profile.recentOrders')}</h3>
                  <Link to="/history" className="text-[10px] font-black text-primary uppercase hover:underline flex items-center gap-1">
                    {t('profile.actions.viewAllOrders')} <span className="material-symbols-outlined text-sm">arrow_forward</span>
                  </Link>
                </div>
                <div className="grid gap-3">
                  {loading ? (
                    <div className="text-center py-8 text-slate-500">
                      ...
                      <p className="text-sm">{t('profile.messages.loading')}</p>
                    </div>
                    ...
                    <div className="text-center py-8 text-slate-500">
                      ...
                      <p className="text-sm">{t('profile.messages.noOrders')}</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      <Modal
        ...
        title={editingAddress ? t('profile.modals.editAddress') : t('profile.modals.addAddress')}
        footer={
          <>
            <Button variant="outline" onClick={() => setIsAddressModalOpen(false)}>{t('profile.actions.cancel')}</Button>
            <Button variant="primary" onClick={saveAddress}>{t('profile.modals.saveAddress')}</Button>
          </>
        }
      >
        <div className="space-y-4">
      ...
      <Modal
        ...
        title={t('profile.modals.changePassword')}
        footer={
          <>
            <Button variant="outline" onClick={() => setIsChangePasswordModalOpen(false)}>{t('profile.actions.cancel')}</Button>
            <Button variant="primary" onClick={handleChangePassword}>{t('profile.modals.updatePassword')}</Button>
          </>
        }
      >
